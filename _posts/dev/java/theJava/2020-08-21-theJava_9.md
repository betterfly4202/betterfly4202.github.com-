...
이제 조금 더 적극적으로 클래스의 행동에 관여하여 DI가 어떻게 이루어지는지 밀접하게 다가가 보자.  
...
`class.newInstance()`방법을 통해서 기본 생성자를 만드는 것이 일반적이었지만, 이 방법은 *deprecated*되었기에 getConstructor를 통해서 인스턴스를 만들었다.  
기본 생성자를 만든다면 **getConstructor(null)**으로 생성하면되고, 위의 경우엔 파라미터가 있는 생성자로 인스턴스를 생성했다.  
주의할점은 bookClass.getConstructor(String.class) 생성자 시점에 파라미터<sub>String.class</sub>받기 때문에, 인스턴스 생성(newInstance)과정에 파라미터를 넣어주지 않으면 파라미터의 수가 맞지 않기 때문에 `IllegalArgumentException`을 만나게된다.

![](/assets/images/study/dev/2020/theJava/9_instance_illgealargument.png)

정상적으로 생성된 인스턴스의 정보를 출력해보면 아래와 같이 정상적으로 접근되는 것을 확인할 수 있다.
...
먼저 *bookName*변수에 접근해보자. 주목할것은 이 변수가 `static`변수라는 것이다. 이미 전체 메모리에 공유되고 있는 static한 변수이다.  
(이 말은 이 변수가 메모리상에서 NewBook class의 bookName이라는 이름으로 공유되고 있다는 것이다.)  
*bookName.get(null)*이 부분이 인스턴스를 주입하는 부분인데, 별도의 인스턴스가 필요없기 때문에 null을 주입했다.  
하지만 static하지 않은 일반 변수 *author*의 경우 접근을 달리해야한다. 이 변수는 어떤 인스턴스에 종속되는지에 대한 명확한 정보가 필요하다.  
