...
소비자의 수요를 예측하고, 어떤 시기에 어떤 재고를 얼마나 보유할 것인지. 이 모든 과정이 철저한 데이터를 토대로 공급망을 관리해아하고 이 모든 기술의 핵심은 오픈소스를 활용하는 것이라며, 다시한 번 오픈소스에 감사의 인사를 전하며 인사를 마쳤다.

곧이어 첫번째 세션인 쿠팡의 정재훈님이 쿠팡이 어떻게 MSA를 도입해 나갔는지의 과정을 풀어주었다.
...
Monolithic의 한계와 어떻게 MSA를 성공적으로 도입했고, 배포부터 관리하는 과정까지 아주 상세히 들을 수 있었다.
